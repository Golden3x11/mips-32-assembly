...
.macro getInt					#funkcja pobjerająca inta od użytkownika
	li $v0, 5				#ustawienie pobrania inta
    	syscall					#pobranie inta
.end_macro

.macro getChar					#funkcja pobjerająca char od użytkownika
	li $v0, 12				#ustawienie pobrania char
    	syscall					#pobranie chara
.end_macro

.macro printCharAndSpace			#funkcja wypisujaca char-a od użytkownika pod podany rejestr
...
.text
main: 
	jal selectNrOfRounds			# Zapytanie się o liczbe rund
...
wrongNrOfRounds:
	printString(errorInputTxt)		# wypisanie błedu
...
selectSymbol: 					# Przypisanie znaku
	...
	printString(wrongSymbolTxt)		# zły symbol 
...
getUserAction:
	...
	add $t6, $s1, $t7			# Adres w pamięci do wybranej pozycji w $t6
...
getComputerAction:
	...
	blt $t7, 32, computerActionLoop2		# 
						...
						# Sprawdzamy w kolejnosci: narożniki środek a potem środki krawedzi
...
checkPosAction:					#albo blokada albo wygrana
	...
	checkPosDiagonal1:			#1 przekątna szukanie miejsca wygranej
	checkPos($s1,0)		
	checkPos($s1,4)
	checkPos($s1,8)
